...
impl Resume<'_, '_, '_> {
    ...
    #[inline(always)]
    pub fn invoke_single_owner(&self, signers: &[Signer]) -> ProgramResult {
        let account_metas = [
    ...
    #[inline(always)]
    pub fn invoke_multisig(&self) -> ProgramResult {
        ...
        invoke_with_bounds::<{ 2 + MAX_MULTISIG_SIGNERS }>(&instruction, unsafe {
            from_raw_parts(acc_infos.as_ptr() as _, num_accounts)
        })
    }
}
